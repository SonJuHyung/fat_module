		      ...
		      struct fat_slot_info *sinfo)
{
	...
	err = fat_scan_dku(dir, msdos_name, sinfo);
			   ...
			   struct timespec *ts, struct fat_slot_info *sinfo)
{
	...
	if (is_hid)
		de.attr |= ATTR_HIDDEN;
	de.lcase = 0;
	fat_time_unix2fat_dku(sbi, ts, &time, &date, NULL);
	de.cdate = de.adate = 0;
	de.ctime = 0;
	de.ctime_cs = 0;
	de.time = time;
	...
	err = fat_add_entries_dku(dir, &de, 1, sinfo);
	if (err)
		return err;
			...
			bool excl)
{
	...
	if (!fat_scan_dku(dir, msdos_name, &sinfo)) {
		brelse(sinfo.bh);
		err = -EINVAL;
		goto out;
	}
...
out:
	mutex_unlock(&MSDOS_SB(sb)->s_lock);
	if (!err)
		err = fat_flush_inodes_dku(sb, dir, inode);
	return err;
}
...
static int msdos_mkdir(struct inode *dir, struct dentry *dentry, umode_t mode)
{
	...
	mutex_unlock(&MSDOS_SB(sb)->s_lock);
	fat_flush_inodes_dku(sb, dir, inode);
	return 0;
			   ...
			   struct dentry *new_dentry, int is_hid)
{
	...
	err = fat_scan_dku(old_dir, old_name, &old_sinfo);
	if (err) {
		err = -EIO;
		goto out;
	}
	...
	err = fat_scan_dku(new_dir, new_name, &sinfo);
	if (!err) {
		if (!new_inode) {
...
out:
	mutex_unlock(&MSDOS_SB(sb)->s_lock);
	if (!err)
		err = fat_flush_inodes_dku(sb, old_dir, new_dir);
	return err;
}
...
{
	return fat_fill_super_dku(sb, data, silent, 0, setup);
}
