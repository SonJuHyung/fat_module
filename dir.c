 ...
 */
int fat_search_long_dku(struct inode *inode, const unsigned char *name,
...
EXPORT_SYMBOL_GPL(fat_search_long_dku);
 ...
 */
int fat_get_dotdot_entry_dku(struct inode *dir, struct buffer_head **bh,
			 struct msdos_dir_entry **de)
{
	loff_t offset = 0;
...
EXPORT_SYMBOL_GPL(fat_get_dotdot_entry_dku);

/* See if directory is empty */
int fat_dir_empty_dku(struct inode *dir)
{
	struct buffer_head *bh;
	struct msdos_dir_entry *de;
	loff_t cpos;
...
EXPORT_SYMBOL_GPL(fat_dir_empty_dku);
 ...
 */
int fat_scan_dku(struct inode *dir, const unsigned char *name,
...
EXPORT_SYMBOL_GPL(fat_scan_dku);
...
int fat_remove_entries_dku(struct inode *dir, struct fat_slot_info *sinfo)
{
	struct super_block *sb = dir->i_sb;
	struct msdos_dir_entry *de;
	struct buffer_head *bh;
...
EXPORT_SYMBOL_GPL(fat_remove_entries_dku);
...
int fat_alloc_new_dir_dku(struct inode *dir, struct timespec *ts)
{
	struct super_block *sb = dir->i_sb;
	struct msdos_sb_info *sbi = MSDOS_SB(sb);
...
EXPORT_SYMBOL_GPL(fat_alloc_new_dir_dku);
...
error_nomem:
	for (i = 0; i < n; i++)
		bforget(bhs[i]);
	fat_free_clusters_dku(dir, cluster[0]);
error:
	return err;
}

int fat_add_entries_dku(struct inode *dir, void *slots, int nr_slots,
		    struct fat_slot_info *sinfo)
{
	struct super_block *sb = dir->i_sb;
	struct msdos_sb_info *sbi = MSDOS_SB(sb);
...
EXPORT_SYMBOL_GPL(fat_add_entries_dku);
