...
int fat_add_cluster(struct inode *inode)
{
	...
	if (err)
		fat_free_clusters_dku(inode, cluster);
	return err;
}
...
void fat_attach_dku(struct inode *inode, loff_t i_pos)
{
...
EXPORT_SYMBOL_GPL(fat_attach_dku);

void fat_detach_dku(struct inode *inode)
{
	struct msdos_sb_info *sbi = MSDOS_SB(inode->i_sb);
	spin_lock(&sbi->inode_hash_lock);
...
EXPORT_SYMBOL_GPL(fat_detach_dku);
...
struct inode *fat_build_inode_dku(struct super_block *sb,
			struct msdos_dir_entry *de, loff_t i_pos)
{
	...
	fat_attach_dku(inode, i_pos);
	insert_inode_hash(inode);
out:
	fat_unlock_build_inode(MSDOS_SB(sb));
	return inode;
}
EXPORT_SYMBOL_GPL(fat_build_inode_dku);
...
{
	fat_inode_cachep = kmem_cache_create("fat_inode_cache_dku",
...
retry:
	...
	fat_time_unix2fat_dku(sbi, &inode->i_mtime, &raw_entry->time,
			  &raw_entry->date, NULL);
	if (sbi->options.isvfat) {
		__le16 atime;
		fat_time_unix2fat_dku(sbi, &inode->i_ctime, &raw_entry->ctime,
				  &raw_entry->cdate, &raw_entry->ctime_cs);
		fat_time_unix2fat_dku(sbi, &inode->i_atime, &atime,
...
int fat_sync_inode_dku(struct inode *inode)
{
	return __fat_write_inode(inode, 1);
}
EXPORT_SYMBOL_GPL(fat_sync_inode_dku);
 ...
 */
int fat_fill_super_dku(struct super_block *sb, void *data, int silent, int isvfat,
...
EXPORT_SYMBOL_GPL(fat_fill_super_dku);
 ...
 * io over again
 */
int fat_flush_inodes_dku(struct super_block *sb, struct inode *i1, struct inode *i2)
{
	int ret = 0;
...
EXPORT_SYMBOL_GPL(fat_flush_inodes_dku);
